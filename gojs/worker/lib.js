...
var WanixHandle = class {
  ...
  async bindCowFS(base, overlay, target2, whiteout = ".wh") {
    this.logger(`bindCowFS ${base} ${overlay} ${target2} ${whiteout}`);
    await this.peer.call("BindCowFS", [base, overlay, target2, whiteout]);
  }
  async bindMemFS(target2) {
    this.logger(`bindMemFS ${target2}`);
    await this.peer.call("BindMemFS", [target2]);
  }
  ...
  async exportBundle(filesystems = []) {
    ...
    for (const desc of manifest.filesystems || []) {
      ...
      if (filesystemSource && filesystemSource !== target2) {
  ...
  async exportPathBundle(path, options = {}) {
    const target2 = options.target || path;
    const id = options.id || "rootfs";
    const filesystems = [{
      id,
      source: options.source || ".",
      archive: path,
      archive_target: target2,
    ...
    if (options.state === false) {
      const manifest = normalizeBundleManifest({
        ...
        filesystems: filesystems.map(normalizeBundleFilesystem).map(({ archive, archive_target, filesystem_source, ...desc }) => ({
          ...desc,
          source: filesystem_source || target2
        })),
        tasks: [],
        vms: []
      });
  ...
  async importBundle(bundle, options = {}) {
    ...
    for (const archive of bundle.archives || []) {
      ...
      const filesystemSource = archive.filesystem_source || target2;
    ...
    try {
      for (const state of bundle.states || []) {
        const target2 = state.path || state.state_path;
        if (!target2) {
        ...
        if (kind === "task") {
          ...
          if (task && task.state_path !== target2) {
            task.state_path = target2;
          }
        } else {
          const vm = (manifest.vms || []).find((vm2) => vm2.id === state.id);
          if (vm && vm.state_path !== target2) {
            vm.state_path = target2;
          }
        }
        await this.writeFile(target2, bundleStateData(state.data));
        cleanup.push(target2);
      }
      return await this.restoreBundleManifest(manifest);
    } finally {
      for (const target2 of cleanup.reverse()) {
        try {
          await this.remove(target2);
        } catch {
        }
      }
    }
  ...
  async replaceBundleTargets(bundle) {
    ...
    for (const archive of bundle.archives || []) {
      ...
      const target2 = bundleArchiveTarget(desc, archive) || archive.source || desc && desc.source;
      if (target2) {
        try {
          await this.removeAll(target2);
        } catch {
        }
      }
    }
