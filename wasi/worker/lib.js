...
var WanixHandle2 = class {
  ...
  async importBundle(bundle, options = {}) {
    ...
    try {
      ...
      for (const target3 of cleanup.reverse()) {
        try {
...
async function postWanixCheckpointSaveState(message, options = {}) {
  const target3 = options.target || globalThis;
  const save = options.save || globalThis.wanixCheckpointState;
  target3.postMessage(await wanixCheckpointSaveStateResponse(message, save));
}
...
function installWanixCheckpoint(options = {}) {
  ...
  target3.addEventListener("message", async (event) => {
